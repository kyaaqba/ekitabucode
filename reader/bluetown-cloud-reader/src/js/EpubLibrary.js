...
Helpers){
    ...
    /**
     * Given a fieldname, sort a list of epubs based on that field.
     *
     */
    var sortEpubs = function(epubs,fieldName) {
        //if it is a book title we don't want to sort based on articles, eg, "the"
        if (fieldName == "title")
        {
            epubs.sort(function(a,b){
                if (removeArticles(a[fieldName]) < removeArticles(b[fieldName])) return -1;
                if (removeArticles(a[fieldName]) > removeArticles(b[fieldName])) return 1;
                return 0;
            });
        }
    ...
    var loadLibraryItems = function(epubs){
        ...
        //Check if we have a sort value in the URL 
        var url_string = window.location.href;
        var url = new URL(url_string);
        var sortParameter = url.searchParams.get("sort");


        //Check if sort value is in our allowed sort values
        var allowedSortValues = [ "title", "author" ];
        if (allowedSortValues.includes(sortParameter))
        {
            sortEpubs(epubs,sortParameter);
        }
    ...
    var readClick = function(e){
        ...
        if (ebookURL || pdfURL) {
            var eventPayload = {embedded: embedded, epub: ebookURL, pdf: pdfURL, epubs: libraryURL};
            $.post("/epub_content?canViewBook=1&uri=" + encodeURIComponent(ebookURL), function(data){
                var result = eval('(' + data + ')');
                var title = "Book Not Loaded";
                if (!result.authorized)
                {
                    var dialogText = "You are currently not able to read this book.  " + "\n" +
                            result.messages;
                    Dialogs.showErrorWithDetails(title, dialogText);
                }
                else {
                    if (ebookURL) {
                        ...
                        $(window).triggerHandler('readpdf', eventPayload);
                    }
                }
            });
        }
    ...
    var loadLibraryUI = function(){
        ...
        $('#library-select').change(function() {
            ...
            {
                var get_parameter = encodeURI("../../epub_content/"+library_name+".json");
                var url_base = window.document.location.protocol + 
                    "//" +
