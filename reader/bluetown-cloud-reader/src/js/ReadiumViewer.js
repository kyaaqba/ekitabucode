define(['jquery', './EpubLibrary', './EpubReader', 'readium_shared_js/helpers', 'URIjs'], function($, EpubLibrary, EpubReader, Helpers, URI){
    ...
    $(window).on('readepub', function(e, eventPayload){
     ...
     $(window).on('readpdf',function(e, eventPayload){
        console.log('readpdf click');
        if (!eventPayload || !eventPayload.pdf) return;

        var ebookURL_filepath = Helpers.getEbookUrlFilePath(eventPayload.pdf);

        var pdf = eventPayload.pdf;
        if (pdf && (typeof pdf !== "string")) {
            pdf = ebookURL_filepath;
        }
        ...
        EpubReader.loadPdf(eventPayload);
    });
