...
BookmarkData){
    ...
    $('#epubContentIframe').on('load', function(){
        console.log('iframe loaded, jquery detected it');
        $(this).contents().find('html').on('click',  function(e){
            //Do some stuff
            console.log('iframe HTML clicked');
        });
    });
...
//        var KEY_RIGHT = 0x27;
    ...
    var loadReaderPdf = function(data){
        ...
        $('#app-navbar').addClass("pdf-nav");
        installReaderEventHandlers();
        document.title = "Readium";
        console.log(data);
        $appContainer.append("<iframe style='width: 100%; height: 100%;' src='" + data.pdf + "'></iframe>");
        ...
        var $pageBtnsContainer = $('#app-navbar');
        $pageBtnsContainer.append(ReaderBodyPageButtons({strings: Strings, dialogs: Dialogs, keyboard: Keyboard,
                    pageProgressionDirectionIsRTL: "rtl"
                }));
    }
