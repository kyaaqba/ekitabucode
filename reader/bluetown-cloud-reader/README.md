...
The `.env` file at the root of the project constitutes database credentials to connect with the database set up to store usage data from the cloud reader. Add values for the following variables in the `.env` to connect to your databse:

* database
* user
* pass 

Provide the name of the database host in the `TrafficDirector.php` (at root directory), within the prepared statement by modifying the value of the servername variable:

* $servername

To stream the usage data into the designated table, replace `usage_log` in `function.php` (at root directory) with the name of your table in the database.Your table in the database created should have the following fields:

* id: INTEGER
* ip: STRING
* within_concurrent_user_limit: INTEGER
* within_allowed_ip: INTEGER
* session_id: STRING
